...
import sys, json
from agentrace.intelligence import analyse_trace
...
export function migrateLegacyTraces() {
  const legacyDir = join(homedir(), '.agentrace', 'traces');
  ...
  if (migrated > 0) console.log(`[AgentTrace] Migrated ${migrated} legacy traces to dashboard`);
}
